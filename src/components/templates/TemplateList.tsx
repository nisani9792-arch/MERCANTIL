...
import { BanknoteArrowDown, BanknoteArrowUp, CalendarClock, Edit3, Plus, Power, Trash2 } from "lucide-react";
import { useState } from "react";
import { BottomSheet } from "@/components/ui/BottomSheet";
import { formatCurrency } from "@/lib/utils/format";
import type { RecurringTemplate } from "@/types/ledger";

type Draft = { id?: string; name: string; amount: string; type: "income" | "expense"; frequency: "monthly" | "bi-monthly"; dayOfMonth: string; isActive: boolean };
const EMPTY_DRAFT: Draft = { name: "", amount: "", type: "expense", frequency: "monthly", dayOfMonth: "", isActive: true };
...
export function TemplateList() {
  ...
  }, onSuccess: async () => { setDraft(null); setMessage("נשמר בהצלחה"); await qc.invalidateQueries({ queryKey: ["templates"] }); window.setTimeout(() => setMessage(""), 2200); }});
  ...
  const edit = (item: RecurringTemplate) => setDraft({ id: item.id, name: item.name, amount: String(item.amount), type: item.type, frequency: item.frequency, dayOfMonth: item.day_of_month ? String(item.day_of_month) : "", isActive: item.is_active });
  ...
  return <div className="mx-auto max-w-5xl space-y-5 pb-8">
    ...
    <BottomSheet open={Boolean(draft)} onClose={() => setDraft(null)} title={draft?.id ? "עריכת פריט קבוע" : "פריט קבוע חדש"}>{draft && <TemplateForm draft={draft} setDraft={setDraft} saving={saveMut.isPending} onSave={() => saveMut.mutate(draft)} />}</BottomSheet>
  </div>;
}
...
function TemplateForm({ draft, setDraft, saving, onSave }: { draft: Draft; setDraft: (value: Draft) => void; saving: boolean; onSave: () => void }) {
  const day = draft.dayOfMonth ? Number(draft.dayOfMonth) : null; const valid = draft.name.trim() && Number(draft.amount) > 0 && (day === null || (day >= 1 && day <= 31));
  return <div className="grid gap-4"><div className="grid grid-cols-2 gap-2 rounded-2xl bg-surface-container p-1">{(["expense", "income"] as const).map((type) => <button key={type} type="button" onClick={() => setDraft({ ...draft, type })} className={`min-h-11 rounded-xl text-sm font-bold ${draft.type === type ? "bg-surface-container-lowest text-primary shadow-elevation-1" : "text-on-surface-variant"}`}>{type === "expense" ? "הוצאה" : "הכנסה"}</button>)}</div><label><span className="m3-label">שם הפריט</span><input autoFocus className="m3-input mt-1 w-full px-3 py-3" value={draft.name} onChange={(e) => setDraft({ ...draft, name: e.target.value })} placeholder="למשל: שכירות" /></label><div className="grid grid-cols-2 gap-3"><label><span className="m3-label">סכום</span><input className="m3-input mt-1 w-full px-3 py-3" type="number" inputMode="decimal" dir="ltr" value={draft.amount} onChange={(e) => setDraft({ ...draft, amount: e.target.value })} /></label><label><span className="m3-label">יום בחודש</span><input className="m3-input mt-1 w-full px-3 py-3" type="number" min="1" max="31" inputMode="numeric" dir="ltr" value={draft.dayOfMonth} onChange={(e) => setDraft({ ...draft, dayOfMonth: e.target.value })} placeholder="אופציונלי" /></label></div><label><span className="m3-label">תדירות</span><select className="m3-input mt-1 w-full px-3 py-3" value={draft.frequency} onChange={(e) => setDraft({ ...draft, frequency: e.target.value as Draft["frequency"] })}><option value="monthly">כל חודש</option><option value="bi-monthly">כל חודשיים</option></select></label><label className="flex min-h-12 items-center gap-3 rounded-xl bg-surface-container/50 px-3 text-sm"><input type="checkbox" className="h-5 w-5 accent-primary" checked={draft.isActive} onChange={(e) => setDraft({ ...draft, isActive: e.target.checked })} />פעיל בחודשים חדשים</label><button type="button" disabled={!valid || saving} onClick={onSave} className="m3-btn-primary min-h-[52px] w-full">{saving ? "שומר..." : "שמירת הפריט"}</button></div>;
}
